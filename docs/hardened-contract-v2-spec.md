...
4. Allocation remains claim-based. To avoid a second unbounded credit loop, store final round parameters and let `claim(roundId, ticketIndex)` prove one ticket's entitlement once, using a claimed bitmap/mapping. The local reference rounds the aggregate award down to a multiple of the finalist count and returns the deterministic remainder to the jackpot reserve; this prevents unclaimable liability dust.
