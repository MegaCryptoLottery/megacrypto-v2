...
async function activateCalendar(x, offset = MONDAY_0000_UTC) {
  await (await x.lottery.proposeWeeklyCutoff(offset)).wait();
  await advance(x.provider, 2 * 24 * 60 * 60 + 1);
  await (await x.lottery.activateWeeklyCutoff()).wait();
}
...
describe('production candidate weekly anchored calendar', function () {
  it('uses the configured UTC weekday/clock cutoff and retains 50/38/6/6 purchase accounting', async () => {
    const x = await fixture(); await activateCalendar(x);
    await completeAndOpenNext(x);
    const round = await x.lottery.rounds(2);
    ...
    assert.equal((await x.lottery.rounds(2)).weeklyPool, 1_900_000n);
  ...
  it('keeps successive rounds exactly one scheduled week apart despite delayed openings', async () => {
    const x = await fixture(); await activateCalendar(x); await completeAndOpenNext(x);
    const cutoff2 = (await x.lottery.rounds(2)).cutoffAt;
    await advance(x.provider, Number(cutoff2 - BigInt((await x.provider.getBlock('latest')).timestamp)) + 2 * 24 * 60 * 60);
    await (await x.lottery.closeRound(2)).wait(); await (await x.lottery.openNextRound()).wait();
    const cutoff3 = (await x.lottery.rounds(3)).cutoffAt;
    assert.equal(cutoff3, cutoff2 + BigInt(WEEK));
    await advance(x.provider, Number(cutoff3 - BigInt((await x.provider.getBlock('latest')).timestamp)) + 3 * 24 * 60 * 60);
    await (await x.lottery.closeRound(3)).wait(); await (await x.lottery.openNextRound()).wait();
    assert.equal((await x.lottery.rounds(4)).cutoffAt, cutoff3 + BigInt(WEEK));
  });
  ...
  it('opens at the next scheduled cutoff after a delayed VRF/settlement lifecycle', async () => {
    ...
    await (await x.lottery.closeRound(2)).wait(); await (await x.lottery.requestRandomness(2)).wait();
    await advance(x.provider, 5 * 24 * 60 * 60); await (await x.coordinator.fulfill(1, 42)).wait(); await (await x.lottery.processSettlement(2, 1)).wait(); await (await x.lottery.openNextRound()).wait();
    assert.equal((await x.lottery.rounds(3)).cutoffAt, cutoff2 + 2n * BigInt(WEEK));
  });
  ...
  it('time-locks and validates calendar changes without altering the active round cutoff', async () => {
    ...
    await (await x.lottery.proposeWeeklyCutoff(MONDAY_0000_UTC)).wait();
    ...
    assert.equal((await x.lottery.rounds(1)).cutoffAt, activeCutoff);
    await completeAndOpenNext(x);
    assert.equal((await x.lottery.rounds(2)).cutoffAt % BigInt(WEEK), BigInt(MONDAY_0000_UTC));
  });
});
