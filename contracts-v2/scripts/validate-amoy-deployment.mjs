...
import { AMOY, candidateArtifact, frozen, hash } from './amoy-config.mjs';

const address = process.env.AMOY_LOTTERY_ADDRESS;
if (!address) { console.log(JSON.stringify({ mode: 'DRY_RUN_READ_ONLY', required: ['AMOY_LOTTERY_ADDRESS', 'AMOY_MOCK_USDT_ADDRESS', 'AMOY_SUBSCRIPTION_ID', 'AMOY_EMERGENCY_AUTHORITY'], expected: AMOY, frozenHashes: frozen }, (_, value) => typeof value === 'bigint' ? value.toString() : value, 2)); process.exit(0); }
