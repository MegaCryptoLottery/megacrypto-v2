...
export interface TransactionReview { kind: 'approval' | 'ticket'; network: string; contract: string; lottery: string; token: string; tokenAddress: string; decimals: number; amount: bigint; numbers: number[]; allowance: bigint; gas?: bigint; gasLimit?: bigint; request: TransactionRequest }
export async function prepareTicketPurchase(chain: ChainConfig, provider: ReturnType<NonNullable<import('./wallet').WalletController['getProvider']>>, numbers: number[]): Promise<TransactionReview> {
  ...
  const gas = await signer.estimateGas(request); const gasLimit = gas + gas / 5n;
  return { kind: approvalNeeded ? 'approval' : 'ticket', network: chain.name, contract: approvalNeeded ? chain.contracts.token! : chain.contracts.lottery, lottery: chain.contracts.lottery, token: chain.contracts.tokenSymbol, tokenAddress: chain.contracts.token!, decimals: chain.contracts.tokenDecimals, amount, numbers: [...numbers].sort((a,b) => a-b), allowance, gas, gasLimit, request: { ...request, gasLimit } };
}
