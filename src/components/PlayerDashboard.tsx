...
export function PlayerDashboard({ chain, state, wallet }: { chain: ChainConfig; state?: SelectedNetworkState; wallet?: string }) {
  ...
  return <section id="tickets" className="player-dashboard panel" aria-label="My tickets and rewards">
    <div><p className="eyebrow">MY TICKETS · {chain.name.toUpperCase()}</p><h2>Current on-chain tickets</h2><small>{state ? `${state.scannedBets} of ${state.currentBets} current bets read from the verified getter` : 'Connect a wallet to read verified current-week tickets.'}</small></div>
    ...
    <p className="data-limitation">Ticket history is reconstructed from the current `apostasDaSemana` getter. Older rounds are not inferred because the verified ABI does not expose historical ticket selections.</p>
  </section>;
}
