...
export function DrawPrizePanel({ jackpot, weeklyPool, ticketPrice, network, chainId, numbers, closesAt }: { jackpot: string; weeklyPool: string; ticketPrice: string; network: string; chainId: number; numbers: number[]; closesAt?: Date }) {
  const cutoff = roundCutoff(closesAt);
  return (
    <aside className="panel draw-prizes" aria-label="Draw and prize information">
      <section className="next-draw"><p className="eyebrow">◷ CURRENT ROUND <b>{cutoff ? 'LIVE CUTOFF' : 'NOT EXPOSED'}</b></p><h2>{cutoff ? 'Closes' : '—'} <small>{cutoff ?? 'Schedule not exposed on-chain'}</small></h2></section>
